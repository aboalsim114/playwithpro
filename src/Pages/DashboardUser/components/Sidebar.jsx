...
const Sidebar = ({ 
  ...
  return (
    <aside 
      className={`fixed left-0 top-0 h-full bg-gray-800 border-r border-gray-700 z-40 overflow-y-auto transition-all duration-300 ${
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="p-6 border-b border-gray-700">
          <div className="text-center">
            {!isCollapsed ? (
              ...
              <div className="text-xl font-bold text-blue-400">PWP</div>
            )}
          </div>
          {/* Bouton pour basculer l'état collapsed */}
          <button
            onClick={onToggleCollapse}
            className="absolute top-4 right-4 p-2 text-gray-400 hover:text-white transition-colors"
            aria-label={isCollapsed ? "Expandir le menu" : "Réduire le menu"}
          >
        ...
        <div className="p-4 border-t border-gray-700">
          ...
          <Link 
            to="/support" 
            className={`w-full bg-gray-700 hover:bg-gray-600 text-gray-300 py-2 px-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 ${isCollapsed ? 'px-2' : ''}`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
